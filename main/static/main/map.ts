...
let __author__ = "Patrick Frontzek";

declare var debug: boolean;
declare var token: string;
declare var urlTrackerSettings: string;
declare var urlDeviceBase: string;
declare var urlTableData: string;
declare var urlDetail: string;
declare var requestUserId: any;
...
interface defaultSettingsInterface {
  ...
  map: {
    ...
    circleMarkerOptions: {
      [key: string]: L.CircleMarkerOptions
    },
  };
  tables: DataTables.Settings,
  dateTimePicker: any,
  tooltip?: Bootstrap.TooltipOption,
  modal?: Bootstrap.ModalOption,
}

const defaults: defaultSettingsInterface = {
  url: {
...
var lastXhr: XMLHttpRequest = null;
var imei = 0;
var start = 0;
var end = 0;
...
const modalMap = $("#mapModal");
const modalTracker = $("#modalTrackerEdit");
const cardStatus = $("#cardStatus");
let cardBodyTableStatus = $("#cardStatusBody");
let cardBodyTableTracker = $("#cardTrackerBody");
const progressTableStatus = $("#progressTableStatus");
const progressTableTracker = $("#progressTableTracker");
const spanModalMapTimestamp = $("#modalMapTimestamp");
const bttnModalTrackerOpen = $("#bttnModalTrackerOpen");
const bttnModalTrackerSave = $("#bttnModalTrackerSave");
const bttnModalMapOpen = $("#bttnModalMapOpen");
const bttnModalMapNext = $("#bttnModalMapNext");
const bttnModalMapPrevious = $("#bttnModalMapPrevious");
const tableTracker = $('#trackerTable').DataTable({
  ...JSON.parse(JSON.stringify(defaults.tables)),
  dom: `l f tr i p`,
  ajax: {
...
const tableStatus = $('#statusTable').DataTable({
  ...JSON.parse(JSON.stringify(defaults.tables)),
  dom: `B l f tr i p`,
  buttons: {
    dom: {
    ...
    },
    {
      ...
      action: function (e, dt, node, config) {
        ...
        let url = `${defaults.url.deviceBase}${imei}/export/csv?start=${start}&end=${end}`;
      ...
      //         let url = urlDeviceBase + imei + "/export/gpx?start=" + start + "&end=" + end;
      //         link.setAttribute("href", url);
      //         link.setAttribute("download", url);
      //         link.click();
      //         link.remove();
...
const webSocket = new WebSocket(defaults.url.websocket);

$('.modal').modal(defaults.modal);


const mapMain = L.map('karte').setView(defaults.map.latLng, defaults.map.zoom);
L.tileLayer(defaults.map.tileUrlTemplate, defaults.map.tileOptions).addTo(mapMain);
const layerGroupMapMain = L.layerGroup().addTo(mapMain);
...
function selectNext(table: DataTables.Api) {
...
function selectPrevious(table: DataTables.Api) {
  var length = table.rows({ selected: true }).length;
  var newId = parseInt(table.rows({ selected: true })[length - 1]) - 1;
  console.log(newId);
  table.rows().deselect();
...
webSocket.onmessage = (ev) => {
  console.log(ev.data);
  let device = JSON.parse(ev.data).device;
  if (device === imei) {
    let refreshButton = $(".refreshButton");
...
let fillPlaceholder = (e: JQuery<HTMLElement>, p: JQuery<HTMLElement>, sub: string = null) => {
  const attrs = ["class", "placeholder", "id", "style"];
  if (sub) {
    attrs.forEach(attr => {
...
tableTracker.on("init", function () {
  let wrapper = $("#trackerTable_wrapper");
  ...
  if (debug) {
    modalMap.on("shown.bs.modal", () => { mapModal.invalidateSize(); });

    bttnModalMapOpen.on("click", () => { modalMap.modal("show"); });

    bttnModalTrackerSave.on("click", () => {
      $.ajax({
    ...
    modalTracker.on("shown.bs.modal", () => {
      $.ajax({
        url: defaults.url.trackerSettings + imei,
        method: "GET",
        headers: {
...
tableStatus.on("init", function () {
  ...
  let dateTimePickerStart: any = $("#dateTimePickerStart");
  ...
  let dateTimePickerEnd: any = $("#dateTimePickerEnd");
  ...
  dateTimePickerStart.on("change.datetimepicker", (e) => {
  ...
  dateTimePickerEnd.on("change.datetimepicker", (e) => {
...
tableStatus.on('select', function (e, dt, type, indexes) {
  ...
  $.ajax({
    ...
    L.circle([data.lat, data.lon], {
      ...defaults.map.circleMarkerOptions.red,
      radius: data.radius * 1000
    }).addTo(layerGroupMapMain);
    if (debug) {
      L.circle([data.lat, data.lon], {
        ...defaults.map.circleMarkerOptions.red,
        radius: data.radius * 1000
      }).addTo(layerGroupMapModal);
      $.each(data.cells, (i, cell) => {
        L.circle([cell.lat, cell.lon], {
          ...defaults.map.circleMarkerOptions.green,
          radius: cell.radius
        }).addTo(layerGroupMapMain);
        L.circle([cell.lat, cell.lon], {
          ...defaults.map.circleMarkerOptions.green,
...
bttnModalMapPrevious.on("click", () => {
  selectPrevious(tableStatus);
});

bttnModalMapNext.on("click", () => {
  selectNext(tableStatus);
});